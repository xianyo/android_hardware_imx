...
status_t Tc358743::changeSensorFormats(int *src, int len)
{
    if (src == NULL || len == 0) {
        return 0;
    }
...
status_t Tc358743::initSensorInfo(const CameraInfo& info)
{
    if (mCameraHandle < 0) {
        FLOGE("Tc358743: initParameters sensor has not been opened");
        return BAD_VALUE;
    }
    ...
    // v4l2 enum format is wrong so hard code here.
    ...
    while (ret == 0) {
        ...
        if (ret == 0) {
            ...
            // v4l2 driver does not support VIDIOC_ENUM_FRAMEINTERVALS so
            // hard code framerate here.
            if (ret == 0) {
...
status_t Tc358743::adjustPreviewResolutions()
{
...
status_t Tc358743::setMaxPictureResolutions()
{
    int xMax, yMax;
